...
class RanForOverviewPage extends StatefulWidget {
  const RanForOverviewPage(
      {super.key,
      this.fileName,
      this.dataset,
      this.metadata,
  ...
  @override
  State<RanForOverviewPage> createState() => _RanForOverviewPageState();
}

class _RanForOverviewPageState extends State<RanForOverviewPage>
  ...
  Future<void> loadModel() async {
    ...
    try {
      final response = await http.post(
          Uri.parse('http://127.0.0.1:5000/load_random_forest'),
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['feature_importance'] != null) {
          var tempMap =
              responseData['feature_importance'] as Map<dynamic, dynamic>;
          featureImportance = tempMap
              .map((key, value) => MapEntry(key.toString(), value.toDouble()));
        }
        int estimators = responseData['estimators'] != null
            ? int.tryParse(responseData['estimators'].toString()) ?? 1
            : 1;
        List<String> classes = responseData['classes'] != null
            ? List<String>.from(responseData['classes'])
            : [];
  ...
  Future<void> trainRanFor() async {
    ...
    final body = numTrees.text.isNotEmpty
        ? {
            'features': finalFeatures.toList(),
            'target': finalTarget,
            'dataset': data,
            'n_trees': numTrees.text
          }
        : {
            'features': finalFeatures.toList(),
            'target': finalTarget,
    ...
    try {
      final response = await http.post(
          Uri.parse('http://127.0.0.1:5000/train_random_forest'),
          headers: {'Content-Type': 'application/json'},
          body: json.encode(body));
  ...
  Future<void> runInference(
    ...
    final url = Uri.parse('http://127.0.0.1:5000/infer_random_forest');
    try {
    ...
    return Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 10),
            ...widget.classes.map((classification) => Text(classification,
                style: const TextStyle(
...
class _VisualizeTreesSection extends State<VisualizeTreesSection> {
  ...
  Future<void> visualizeTree() async {
    ...
    final response = await http.post(
        Uri.parse('http://127.0.0.1:5000/graph_random_forest'),
        headers: {'Content-Type': 'application/json'},
        body: json.encode(body));
